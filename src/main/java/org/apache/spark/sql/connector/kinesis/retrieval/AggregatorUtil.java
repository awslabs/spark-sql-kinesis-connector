...
public class AggregatorUtil {
  private final Logger log = LoggerFactory.getLogger(AggregatorUtil.class);
  ...
  private static final BigInteger ENDING_HASH_KEY = new BigInteger(
   ...
   */
  public List<KinesisUserRecord> deaggregate(List<Record> records, Long millisBehindLatest) {
    return deaggregate(records, STARTING_HASH_KEY, ENDING_HASH_KEY, millisBehindLatest);
  }
   ...
   */
  public List<KinesisUserRecord> deaggregate(List<Record> records, String startingHashKey,
                                               String endingHashKey, Long millisBehindLatest) {
    return deaggregate(records,
        new BigInteger(startingHashKey),
        new BigInteger(endingHashKey),
        millisBehindLatest
    );
  }
  ...
  public List<KinesisUserRecord> deaggregate(List<Record> records,
                                               BigInteger startingHashKey,
                                               BigInteger endingHashKey,
                                               Long millisBehindLatest
                                               ) {
    List<KinesisUserRecord> result = new ArrayList<>();
    ...
    for (Record r : records) {
      ...
      long subSeqNum = 0;
      ByteBuffer bb = r.data().asByteBuffer();
      ...
      log.debug("deaggregate(): isAggregated value " + isAggregated + ", record SequenceNumber:"
          + r.sequenceNumber() + ", ApproximateArrivalTimestamp: " + r.approximateArrivalTimestamp()
        ...
        if (!Arrays.equals(digest, calculatedDigest)) {
          ...
          try {
            ...
            try {
              ...
              int totalMRs = ar.getRecordsList().size();
              log.debug("deaggregate(): isAggregated totalMRs " + totalMRs);
              for (Messages.Record mr : ar.getRecordsList()) {
                ...
                if (effectiveHashKey.compareTo(startingHashKey) < 0
                  ...
                  log.warn("effectiveHashKey "+ effectiveHashKey +" not in range (" + startingHashKey
                      + ", " + endingHashKey +")");
                ...
                Boolean isLast = (totalMRs == (subSeqNum + 1));
                result.add(
                    KinesisUserRecord.apply(
                        new SequenceNumber(r.sequenceNumber(), subSeqNum++, isLast),
                        r.approximateArrivalTimestamp(),
                        mr.getData().toByteArray(),
                        partitionKey,
                        r.encryptionType(),
                        millisBehindLatest,
                        true,
      ...
      log.debug("deaggregate(): isAggregated final value " + isAggregated);

      if (!isAggregated) {
        bb.rewind();
        result.add(KinesisUserRecord.apply(r, millisBehindLatest));
      }
    }
    return result;
  }
  ...
  protected BigInteger effectiveHashKey(String partitionKey, String explicitHashKey) throws
